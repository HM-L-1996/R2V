...
public class reviewDAO {
	...
	public ArrayList<reviewVO> getList (String searchType, String search, int pageNumber) {
		...
		try {
			...
			if(searchType.equals("강좌 제목")) { 				
				sql = "select * from review where learn_num in ( select learn_num from learn where learn_title like ?) order by review_num desc limit " + (pageNumber-1) * 10   + ", " + (pageNumber-1) * 10 + 10;
			} else if (searchType.equals("리뷰 내용")) { 
				sql = "select * from review where review_article like ? order by review_num desc limit " + (pageNumber-1) * 10   + ", " + (pageNumber-1) * 10 + 10;
			}
	...
	// 페이지네이션을 위한 카운트 함수
	public int getreviewCount(String searchType, String search) {
		String sql="";
		if(searchType.equals("강좌 제목")) { 
			sql = "select count(review_num) from review where learn_num in (select learn_num from learn where learn_title like ?) ";
		} else if (searchType.equals("리뷰 내용")) { 
			sql = "select count(review_num) from review where review_article like ? ";
		}
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
